...
<!-- BEGIN TOC (generated by tools/gen-dd-toc.py — do not edit by hand) -->

## Contents

_246 entries. Generated — run `python3 tools/gen-dd-toc.py` after adding one._
...
- [Video/office viewers couldn't open a user's files OUTSIDE their home](#videooffice-viewers-couldnt-open-a-users-files-outside-their-home)
- [Files app 403'd a user's own dotfiles ("You don't have permissions to access this")](#files-app-403d-a-users-own-dotfiles-you-dont-have-permissions-to-access-this)
- [The streamed Browser is device-SHAPED (mobile browser on a phone, desktop on a computer)](#the-streamed-browser-is-device-shaped-mobile-browser-on-a-phone-desktop-on-a-computer)
- [Mobile Browser typing: dropped/reordered letters, and Chinese typed nothing](#mobile-browser-typing-droppedreordered-letters-and-chinese-typed-nothing)
- [The mobile Browser lost ALL its patches — `/xpra-patches.js` 404'd after the `/opt` move](#the-mobile-browser-lost-all-its-patches-xpra-patchesjs-404d-after-the-opt-move)
- [Uploads over ~1 MB failed with a 500 (the `auth_request` body-size trap)](#uploads-over-1-mb-failed-with-a-500-the-authrequest-body-size-trap)
- [Porting the installers off apt (and the bug Debian's packaging had been hiding)](#porting-the-installers-off-apt-and-the-bug-debians-packaging-had-been-hiding)
- [The live smoke test reported 12 failures on a healthy multi-user host — and a non-root cookie mint made it look worse](#the-live-smoke-test-reported-12-failures-on-a-healthy-multi-user-host-and-a-non-root-cookie-mint-made-it-look-worse)
- ["Remove" was account deletion sitting next to "Reset password" — and there was no way to just sign someone out](#remove-was-account-deletion-sitting-next-to-reset-password-and-there-was-no-way-to-just-sign-someone-out)
- [Config admin app: sudo gate, and an idle reaper that spares terminals by default](#config-admin-app-sudo-gate-and-an-idle-reaper-that-spares-terminals-by-default)
- [A stale terminal tab name reappears on a new terminal after an abnormal close](#a-stale-terminal-tab-name-reappears-on-a-new-terminal-after-an-abnormal-close)
- [Notes tabs didn't live-sync across devices](#notes-tabs-didnt-live-sync-across-devices)
- [Mobile key bar stuck visible on iPad (but fine on iPhone)](#mobile-key-bar-stuck-visible-on-ipad-but-fine-on-iphone)
- [Black band below the taskbar after a Cloudflare Access login (installed PWA only, iOS)](#black-band-below-the-taskbar-after-a-cloudflare-access-login-installed-pwa-only-ios)
- [GNOME apps (eog, evince) take ~33s to start in the X11 Launcher](#gnome-apps-eog-evince-take-33s-to-start-in-the-x11-launcher)
- [The private X11 D-Bus bus was silently 100%-broken (two config bugs) + a measurement trap](#the-private-x11-d-bus-bus-was-silently-100-broken-two-config-bugs-a-measurement-trap)
- [The REAL evince-slowness: terminal-launched GUI apps were on the real (hanging) bus](#the-real-evince-slowness-terminal-launched-gui-apps-were-on-the-real-hanging-bus)
- [Snap apps (Firefox/Chromium) won't open the X11 display](#snap-apps-firefoxchromium-wont-open-the-x11-display)
- [Browser must stay its own app, but Apps needs its own canvas](#browser-must-stay-its-own-app-but-apps-needs-its-own-canvas)
- [X11 apps launched from a Terminal should appear in the launcher](#x11-apps-launched-from-a-terminal-should-appear-in-the-launcher)
- [eog/evince single-instance hand-off](#eogevince-single-instance-hand-off)
- [`@BASE_PORT@` left unsubstituted in the ttyd unit (latent install bug)](#baseport-left-unsubstituted-in-the-ttyd-unit-latent-install-bug)
- [Tabs in the Files app (multiple folders)](#tabs-in-the-files-app-multiple-folders)
- [Auto-refresh every client on deploy (SSE push)](#auto-refresh-every-client-on-deploy-sse-push)
- [Close an app on all devices](#close-an-app-on-all-devices)
- [Tabs in the Notes app (multiple, renameable notes)](#tabs-in-the-notes-app-multiple-renameable-notes)
- [Launcher "spins forever" on a not-installed / mistyped command](#launcher-spins-forever-on-a-not-installed-mistyped-command)
- [Slow app launch looked broken (blank canvas)](#slow-app-launch-looked-broken-blank-canvas)
- [No server-side logs made debugging slow ("limited logs on the server side")](#no-server-side-logs-made-debugging-slow-limited-logs-on-the-server-side)
- [Killing the terminal "shake" silently broke the double-click/tap re-claim](#killing-the-terminal-shake-silently-broke-the-double-clicktap-re-claim)
- ["Random characters" appear at the prompt (`2RR0;276;0c10;rgb:…$y`)](#random-characters-appear-at-the-prompt-2rr02760c10rgby)
- [macOS "error beep" when copying in a terminal (Cmd+C)](#macos-error-beep-when-copying-in-a-terminal-cmdc)
- [Closing a tab killed detached processes (ssh ControlPersist, tmux, nohup)](#closing-a-tab-killed-detached-processes-ssh-controlpersist-tmux-nohup)
- [Root manager service is only *partially* sandboxed (on purpose)](#root-manager-service-is-only-partially-sandboxed-on-purpose)
- [CSRF on the no-auth manager API (Origin check, not tokens)](#csrf-on-the-no-auth-manager-api-origin-check-not-tokens)
- [`vibetop-session` shell-respawn needs backoff](#vibetop-session-shell-respawn-needs-backoff)
- [Mobile on-screen key bar (arrows/Esc/^C) lives at the TOP, not above the keyboard](#mobile-on-screen-key-bar-arrowsescc-lives-at-the-top-not-above-the-keyboard)
- [Mobile terminal typing: dropped keystrokes, dropped first letter, input lag](#mobile-terminal-typing-dropped-keystrokes-dropped-first-letter-input-lag)
- [OnlyOffice "Download failed" over the Cloudflare tunnel (but fine on the LAN)](#onlyoffice-download-failed-over-the-cloudflare-tunnel-but-fine-on-the-lan)
- [Browser clicks land ~one line low after an xpra restart (xpra 6.5 regression)](#browser-clicks-land-one-line-low-after-an-xpra-restart-xpra-65-regression)
- [Browser "loading" spinner every few seconds with TWO devices (stale xpra client state — NOT a code/version bug)](#browser-loading-spinner-every-few-seconds-with-two-devices-stale-xpra-client-state-not-a-codeversion-bug)
- [Browser stuck at the phone's tiny size on the desktop (no way to re-claim)](#browser-stuck-at-the-phones-tiny-size-on-the-desktop-no-way-to-re-claim)
- [Claude plan-usage strip: capturing the real Max-plan % (there is no query API), and the pinned-session footgun](#claude-plan-usage-strip-capturing-the-real-max-plan-there-is-no-query-api-and-the-pinned-session-footgun)
- [Terminal/Files link → embedded Browser silently stopped opening (RestrictNamespaces vs snap-confine)](#terminalfiles-link-embedded-browser-silently-stopped-opening-restrictnamespaces-vs-snap-confine)
- [Mobile haptics for the arrow-key trackpad — no usable iOS web path (Android only)](#mobile-haptics-for-the-arrow-key-trackpad-no-usable-ios-web-path-android-only)
- [Mobile terminal goes fully blank after `clear` (stale iOS reveal-scroll)](#mobile-terminal-goes-fully-blank-after-clear-stale-ios-reveal-scroll)
- [Mobile terminal: the trackpad slide / a device switch corrupts typing (stale value-diff mirror)](#mobile-terminal-the-trackpad-slide-a-device-switch-corrupts-typing-stale-value-diff-mirror)
- [Mobile terminal resize: two-finger tap, not single-finger double-tap (iOS keyboard conflict)](#mobile-terminal-resize-two-finger-tap-not-single-finger-double-tap-ios-keyboard-conflict)
- [Coach banners: show every time until ×, with a persisted max-showings cap](#coach-banners-show-every-time-until-with-a-persisted-max-showings-cap)
- [Mobile terminal: can't scroll back through a *live* Claude/TUI response (desktop was fine)](#mobile-terminal-cant-scroll-back-through-a-live-claudetui-response-desktop-was-fine)
- [Terminal latest-line recovery must yield to manual scrollback](#terminal-latest-line-recovery-must-yield-to-manual-scrollback)
- [Snap GUI apps fail on the X11 display with "Authorization required" (xhost +local: is not enough)](#snap-gui-apps-fail-on-the-x11-display-with-authorization-required-xhost-local-is-not-enough)
- [Public file-share links (Files app) — punching a hole through Access, safely](#public-file-share-links-files-app-punching-a-hole-through-access-safely)
- [Multi-user auth (Phase 1): Linux-account login, where the gate lives](#multi-user-auth-phase-1-linux-account-login-where-the-gate-lives)
- [Multi-user Phase 3: per-user terminals run AS the logged-in user](#multi-user-phase-3-per-user-terminals-run-as-the-logged-in-user)
- [Multi-user Phase 3b/review: per-user Files + "admin-gate the not-yet-per-user"](#multi-user-phase-3breview-per-user-files-admin-gate-the-not-yet-per-user)
- [Multi-user Phase 3c: per-user Browser + X11 (xpra + snap Chromium as the user)](#multi-user-phase-3c-per-user-browser-x11-xpra-snap-chromium-as-the-user)
- [In-Files video: .mkv "opens but doesn't play", and no audio/subtitle track picker](#in-files-video-mkv-opens-but-doesnt-play-and-no-audiosubtitle-track-picker)
- [Mobile Files app: toolbar, clickable breadcrumb, folder-nav recovery (a long iteration)](#mobile-files-app-toolbar-clickable-breadcrumb-folder-nav-recovery-a-long-iteration)
- [Code-review hardening pass (auth gate, port blocks, and assorted fixes)](#code-review-hardening-pass-auth-gate-port-blocks-and-assorted-fixes)
- [iOS standalone PWA: active line hidden BELOW the screen after reopening from idle](#ios-standalone-pwa-active-line-hidden-below-the-screen-after-reopening-from-idle)
- [Mobile terminal IME: raw pinyin leaked into the shell — extract + unit-test the input state machine](#mobile-terminal-ime-raw-pinyin-leaked-into-the-shell-extract-unit-test-the-input-state-machine)
- [X11 GUI apps (evince/eog) open ~25s slow again — the per-user private D-Bus bus regressed](#x11-gui-apps-evinceeog-open-25s-slow-again-the-per-user-private-d-bus-bus-regressed)
- [Window mode: the chrome kept eating the window's own gestures (v1.19.5–.9)](#window-mode-the-chrome-kept-eating-the-windows-own-gestures-v11959)
- [Window mode round 2 (v1.19.10): the banner, the dead buttons, and the gate](#window-mode-round-2-v11910-the-banner-the-dead-buttons-and-the-gate)
- [The shell must FIT the screen, continuously — not measure it once (v1.19.11)](#the-shell-must-fit-the-screen-continuously-not-measure-it-once-v11911)
- [The 24h schedule history never expired — a correct prune with no idle call site (v1.19.13)](#the-24h-schedule-history-never-expired-a-correct-prune-with-no-idle-call-site-v11913)
- [The keyboard reservation ratcheted: a measurement that included its own output (v1.19.14)](#the-keyboard-reservation-ratcheted-a-measurement-that-included-its-own-output-v11914)
- [Reverted: the whole viewport-fit feature (v1.19.15) — the keyboard premise was wrong](#reverted-the-whole-viewport-fit-feature-v11915-the-keyboard-premise-was-wrong)
- ["Log out" now ends the Cloudflare session too — but only when there is one (v1.19.18)](#log-out-now-ends-the-cloudflare-session-too-but-only-when-there-is-one-v11918)
- [The phone never re-claimed the shared terminal shape on a device switch (v1.19.25)](#the-phone-never-re-claimed-the-shared-terminal-shape-on-a-device-switch-v11925)
- [The first load after the Access login bailed out of the height fix (v1.19.29)](#the-first-load-after-the-access-login-bailed-out-of-the-height-fix-v11929)
- [The desktop rendered inside itself after a re-login](#the-desktop-rendered-inside-itself-after-a-re-login)
- [The one dead resize corner (NE), and why the edges felt like they weren't there](#the-one-dead-resize-corner-ne-and-why-the-edges-felt-like-they-werent-there)
- [The mobile key bar covered the line you type on (and every fix "worked" once)](#the-mobile-key-bar-covered-the-line-you-type-on-and-every-fix-worked-once)
- ["Still cannot resize from left or right" — the tile gutter belonged to nobody](#still-cannot-resize-from-left-or-right-the-tile-gutter-belonged-to-nobody)
- ["No black readout" — instrumenting a device you cannot see](#no-black-readout-instrumenting-a-device-you-cannot-see)
- ["Works in Chrome, not in Safari" — a cursor-MAPPING failure, and the two keywords that survive it](#works-in-chrome-not-in-safari-a-cursor-mapping-failure-and-the-two-keywords-that-survive-it)
- [Making vibetop's resize cursor deliberately UNLIKE the host window's](#making-vibetops-resize-cursor-deliberately-unlike-the-host-windows)
- [A resize cursor needs an explicit handoff at iframe boundaries](#a-resize-cursor-needs-an-explicit-handoff-at-iframe-boundaries)
...
- [One icon, finally: tidying folded into the toggle](#one-icon-finally-tidying-folded-into-the-toggle)
- [Choosing a window layout: snap layouts on the ▢ button](#choosing-a-window-layout-snap-layouts-on-the-button)
- [The layout palette belongs to the desktop, not to a window](#the-layout-palette-belongs-to-the-desktop-not-to-a-window)
- ["Which one is the one in 1 + 2?" — the palette previews, pointing steers](#which-one-is-the-one-in-1-2-the-palette-previews-pointing-steers)
- [Full slot assignment: every steering gesture is a swap, and the preview shows reality](#full-slot-assignment-every-steering-gesture-is-a-swap-and-the-preview-shows-reality)
- [The drag that tested green and shipped broken: repaint churn eats pointerdown](#the-drag-that-tested-green-and-shipped-broken-repaint-churn-eats-pointerdown)
- [The touch resize grip ate the title bar (v1.19.35 → v1.19.56)](#the-touch-resize-grip-ate-the-title-bar-v11935-v11956)
- [The palette becomes a staging area: compose freely, one Apply commits](#the-palette-becomes-a-staging-area-compose-freely-one-apply-commits)
- [The Enter a paste-detecting TUI swallows: scheduled messages typed but never ran](#the-enter-a-paste-detecting-tui-swallows-scheduled-messages-typed-but-never-ran)
- [Files "flashes a few times" on an image — the empty-folder self-heal misread previews](#files-flashes-a-few-times-on-an-image-the-empty-folder-self-heal-misread-previews)
- [Files/Browser come back BLACK after a long idle until clicked](#filesbrowser-come-back-black-after-a-long-idle-until-clicked)
- [Scheduled messages died overnight: the ring outgrew the backpressure kill threshold](#scheduled-messages-died-overnight-the-ring-outgrew-the-backpressure-kill-threshold)
- [`/api/fs/*` shipped with no authentication (local root escalation)](#apifs-shipped-with-no-authentication-local-root-escalation)
- [The file agent's socket could be squatted by another user](#the-file-agents-socket-could-be-squatted-by-another-user)
- [A file named `constructor` joined every selection (Files-native)](#a-file-named-constructor-joined-every-selection-files-native)
- [A `position: fixed` bar anchored at `left: 50%` can only use half the screen](#a-position-fixed-bar-anchored-at-left-50-can-only-use-half-the-screen)
- [The auth fix that stopped one endpoint short](#the-auth-fix-that-stopped-one-endpoint-short)
- [Stop fixing authentication one endpoint at a time](#stop-fixing-authentication-one-endpoint-at-a-time)
- [A level a player cannot finish looks exactly like a level that works](#a-level-a-player-cannot-finish-looks-exactly-like-a-level-that-works)
- [`undefined <= 0` is false, and it has now cost two features](#undefined-0-is-false-and-it-has-now-cost-two-features)
- [Water is a property of the TILE, not of the level](#water-is-a-property-of-the-tile-not-of-the-level)
- [A dismiss button read as a leaderboard, so dismissing read as "nothing happened"](#a-dismiss-button-read-as-a-leaderboard-so-dismissing-read-as-nothing-happened)
- [An overflowing line ignores `text-align`, and it dragged a scrollbar onto the page](#an-overflowing-line-ignores-text-align-and-it-dragged-a-scrollbar-onto-the-page)
- [A recycled pointerId orphaned a d-pad button, and the player walked forever](#a-recycled-pointerid-orphaned-a-d-pad-button-and-the-player-walked-forever)
- [Sweep: every place a pointer could go missing, and every place text could leave the box](#sweep-every-place-a-pointer-could-go-missing-and-every-place-text-could-leave-the-box)
- [The game-over card is a menu, so a stray tap must not answer it](#the-game-over-card-is-a-menu-so-a-stray-tap-must-not-answer-it)
- [The scanner became a test, and it immediately found what Chromium had hidden](#the-scanner-became-a-test-and-it-immediately-found-what-chromium-had-hidden)
- [A chip is one line; what gives is the text inside it, measured](#a-chip-is-one-line-what-gives-is-the-text-inside-it-measured)
- [A stale PWA baselined its version from the server, so it could never notice it was stale](#a-stale-pwa-baselined-its-version-from-the-server-so-it-could-never-notice-it-was-stale)
- [Shed the duration, keep the clock time — I had it backwards](#shed-the-duration-keep-the-clock-time-i-had-it-backwards)
- [A fractional tile index does not throw — it writes somewhere else entirely](#a-fractional-tile-index-does-not-throw-it-writes-somewhere-else-entirely)
- [An AI difficulty knob that nothing reads is not a difficulty](#an-ai-difficulty-knob-that-nothing-reads-is-not-a-difficulty)
- [A screenshot cannot show you that the camera is fighting the player](#a-screenshot-cannot-show-you-that-the-camera-is-fighting-the-player)
- [Copying a behaviour without its stylesheet flashed the whole game](#copying-a-behaviour-without-its-stylesheet-flashed-the-whole-game)
- [An asymmetric faction still has to be able to answer everything](#an-asymmetric-faction-still-has-to-be-able-to-answer-everything)
- [A preference score is not an eligibility threshold](#a-preference-score-is-not-an-eligibility-threshold)
- [Ore is walkable, the ground around it need not be](#ore-is-walkable-the-ground-around-it-need-not-be)
- [A test suite in the pre-commit hook is a shared budget, not a private one](#a-test-suite-in-the-pre-commit-hook-is-a-shared-budget-not-a-private-one)
- [A faction that cannot spend its money loses a game it is winning](#a-faction-that-cannot-spend-its-money-loses-a-game-it-is-winning)
- [Slower than your own infantry is a different unit than "slow"](#slower-than-your-own-infantry-is-a-different-unit-than-slow)
- [Overlays positioned off the footprint drift into taller art](#overlays-positioned-off-the-footprint-drift-into-taller-art)
- [The refinery's dock is on the +gy face, which is screen down-LEFT](#the-refinerys-dock-is-on-the-gy-face-which-is-screen-down-left)
- [Parallel agents must not share a scratch path](#parallel-agents-must-not-share-a-scratch-path)
- [A harvester that re-acquires ore every tick cannot be given an order](#a-harvester-that-re-acquires-ore-every-tick-cannot-be-given-an-order)
- [The AI packed its base because "not too crowded" was the only rule](#the-ai-packed-its-base-because-not-too-crowded-was-the-only-rule)
- [The only saturated red or blue on a sprite is the owner's colour](#the-only-saturated-red-or-blue-on-a-sprite-is-the-owners-colour)
- ["Stretched" buildings: measure the aspect ratio, don't eyeball it](#stretched-buildings-measure-the-aspect-ratio-dont-eyeball-it)
- [RTS combat is RA2's rules.ini, not a three-class multiplier table](#rts-combat-is-ra2s-rulesini-not-a-three-class-multiplier-table)
- [Harrier pads and reloads are a state machine on the unit, not a queue on the building](#harrier-pads-and-reloads-are-a-state-machine-on-the-unit-not-a-queue-on-the-building)
- [`aa` / `ag` weapon flags instead of an "air" armour class](#aa-ag-weapon-flags-instead-of-an-air-armour-class)
- [Aircraft are drawn in a second pass, sorted by ground position](#aircraft-are-drawn-in-a-second-pass-sorted-by-ground-position)
- [Under RA2 pacing the AI has to open Power → Refinery and spend its bank](#under-ra2-pacing-the-ai-has-to-open-power-refinery-and-spend-its-bank)
- [Skewed art in `bakeBuilding` must be point math, not `g.transform`](#skewed-art-in-bakebuilding-must-be-point-math-not-gtransform)
- [The Allied refinery's drum is an open mouth facing the dock, not a hooped end cap](#the-allied-refinerys-drum-is-an-open-mouth-facing-the-dock-not-a-hooped-end-cap)
- [The wiki's "animation" gifs are build-ups: the sprite is the LAST frame](#the-wikis-animation-gifs-are-build-ups-the-sprite-is-the-last-frame)
- [A MAKE gif's last frame can still be wearing its scaffolding](#a-make-gifs-last-frame-can-still-be-wearing-its-scaffolding)
- [Only a RED-owner rip tells you where a sprite's remap actually is](#only-a-red-owner-rip-tells-you-where-a-sprites-remap-actually-is)
- [A structure whose art is a landing pad must be laid out FROM the pad slots](#a-structure-whose-art-is-a-landing-pad-must-be-laid-out-from-the-pad-slots)
- [A repair pad has to be centred on the FOOTPRINT, not where the sprite puts it](#a-repair-pad-has-to-be-centred-on-the-footprint-not-where-the-sprite-puts-it)
- [A sprite's aspect budget is pinned at the bottom by the shared contact shadow](#a-sprites-aspect-budget-is-pinned-at-the-bottom-by-the-shared-contact-shadow)
- [A 1x1 defence cannot wear as much house colour as its RA2 sprite does](#a-1x1-defence-cannot-wear-as-much-house-colour-as-its-ra2-sprite-does)
- [The emitter in the art and the shot's origin constant are two sources of truth](#the-emitter-in-the-art-and-the-shots-origin-constant-are-two-sources-of-truth)
- [RTS infantry: fix proportion with one transform, not 400 hand-edited numbers](#rts-infantry-fix-proportion-with-one-transform-not-400-hand-edited-numbers)
- [The RA2 vehicle sheets are FACING RINGS, and d0 is the SE sprite, not the E one](#the-ra2-vehicle-sheets-are-facing-rings-and-d0-is-the-se-sprite-not-the-e-one)
- [Measuring an RA2 reference sheet: segment by GREENNESS, never by background median](#measuring-an-ra2-reference-sheet-segment-by-greenness-never-by-background-median)
- [MCV deploy-in-place vs the occupancy grid and the build radius](#mcv-deploy-in-place-vs-the-occupancy-grid-and-the-build-radius)
- [A crashing aircraft is a wreck object, not a dying unit](#a-crashing-aircraft-is-a-wreck-object-not-a-dying-unit)
- [A superweapon's timer belongs to the SIDE, and its blast is applied per cell](#a-superweapons-timer-belongs-to-the-side-and-its-blast-is-applied-per-cell)
- [RTS: ramps and bridges are passable by omission, not by a special case](#rts-ramps-and-bridges-are-passable-by-omission-not-by-a-special-case)
- [RTS front menu: pictures instead of prose, and one overlay over the whole frame](#rts-front-menu-pictures-instead-of-prose-and-one-overlay-over-the-whole-frame)
- [Blank build-menu cameos on HiDPI screens: `artBox` scanned the wrong quarter](#blank-build-menu-cameos-on-hidpi-screens-artbox-scanned-the-wrong-quarter)
- [Sidebar tabs follow RA2's BuildCat and TechLevel, not our lanes](#sidebar-tabs-follow-ra2s-buildcat-and-techlevel-not-our-lanes)
- [The RA2 sidebar: five things that are not obvious](#the-ra2-sidebar-five-things-that-are-not-obvious)
- [Structure states are a post-pass over the baked art, never a second hand-drawn sprite](#structure-states-are-a-post-pass-over-the-baked-art-never-a-second-hand-drawn-sprite)
- [RTS controls: cursors, orders, the progressive build charge and an isometric radar](#rts-controls-cursors-orders-the-progressive-build-charge-and-an-isometric-radar)
- [RTS infantry facings are two transforms and five re-cut parts, not nine more branches](#rts-infantry-facings-are-two-transforms-and-five-re-cut-parts-not-nine-more-branches)
- [Prone infantry made the hard Directorate AI lose to Easy; the fix was crushing, not the table](#prone-infantry-made-the-hard-directorate-ai-lose-to-easy-the-fix-was-crushing-not-the-table)
- [Walls, gates and the Gap Generator: three rules that all live on the occupancy grid](#walls-gates-and-the-gap-generator-three-rules-that-all-live-on-the-occupancy-grid)
- [`Adjacent=` is RA2's ratio, not RA2's cell count](#adjacent-is-ra2s-ratio-not-ra2s-cell-count)
- [Civilian blocks are structures owned by house −1, not terrain](#civilian-blocks-are-structures-owned-by-house-1-not-terrain)
- [Bridge spans are columns, and only force-fire brings one down](#bridge-spans-are-columns-and-only-force-fire-brings-one-down)
- [Crates and ore spreading are hashed, not seeded from a stream](#crates-and-ore-spreading-are-hashed-not-seeded-from-a-stream)
- [A tech building near a start is a lump, not a prize](#a-tech-building-near-a-start-is-a-lump-not-a-prize)
- [Explosions are baked FAMILIES, not one blob with a scale factor (2026-09-03)](#explosions-are-baked-families-not-one-blob-with-a-scale-factor-2026-09-03)
- [Ground decals are capped and live on `g.rubble` (2026-09-03)](#ground-decals-are-capped-and-live-on-grubble-2026-09-03)
- [The Lightning Storm fires on RA2's clock, not a spaced-out trickle (2026-09-03)](#the-lightning-storm-fires-on-ra2s-clock-not-a-spaced-out-trickle-2026-09-03)
- [Giving the War Miner its gun conscripted the Collective's economy](#giving-the-war-miner-its-gun-conscripted-the-collectives-economy)
- [Yuri is the one unit with no sprite to build from](#yuri-is-the-one-unit-with-no-sprite-to-build-from)
- [RA2's game-speed slider is ticks per SECOND, not a changed tick](#ra2s-game-speed-slider-is-ticks-per-second-not-a-changed-tick)
- [The save format is JSON with exactly two escapes](#the-save-format-is-json-with-exactly-two-escapes)
- [Picking a house colour re-bakes the sprites, so the click has to say so](#picking-a-house-colour-re-bakes-the-sprites-so-the-click-has-to-say-so)
- [Terrain height is DERIVED from the map, and `sy()` is the only place it lands (2026-09-03)](#terrain-height-is-derived-from-the-map-and-sy-is-the-only-place-it-lands-2026-09-03)
- [LAT edge tiles are cut from a deliberately FLAT sheet (2026-09-03)](#lat-edge-tiles-are-cut-from-a-deliberately-flat-sheet-2026-09-03)
- [The sound engine is sound.ini's rules, not a play() call (2026-09-03)](#the-sound-engine-is-soundinis-rules-not-a-play-call-2026-09-03)
- [Unit acknowledgements are a radio, not a speech synthesiser (2026-09-03)](#unit-acknowledgements-are-a-radio-not-a-speech-synthesiser-2026-09-03)
- [Production doors: one `dopen` argument, not an overlay (2026-09-03)](#production-doors-one-dopen-argument-not-an-overlay-2026-09-03)
- [The AI is a task-force machine now, and its difficulty rests on RA2's economy curve (2026-09-03)](#the-ai-is-a-task-force-machine-now-and-its-difficulty-rests-on-ra2s-economy-curve-2026-09-03)
- [Normal could not beat Easy, and `__rtsSim` could not repeat itself (2026-09-03)](#normal-could-not-beat-easy-and-rtssim-could-not-repeat-itself-2026-09-03)
- [RA2 `Foundation=` footprints (Phase 2)](#ra2-foundation-footprints-phase-2)
- [The aspect harness was measuring the shadow (art pass 9)](#the-aspect-harness-was-measuring-the-shadow-art-pass-9)
- [Chokepoint Pass: a ramp that climbs nothing (art pass 9)](#chokepoint-pass-a-ramp-that-climbs-nothing-art-pass-9)
- [Transports: a passenger is a record, not a hidden unit (2026-09-03)](#transports-a-passenger-is-a-record-not-a-hidden-unit-2026-09-03)
- [The IFV's weapon is a property of its passenger, resolved in `weaponFor` (2026-09-03)](#the-ifvs-weapon-is-a-property-of-its-passenger-resolved-in-weaponfor-2026-09-03)
- [Two art traps the transports fell into (2026-09-03)](#two-art-traps-the-transports-fell-into-2026-09-03)
- [32 facings for a voxel, 8 for an SHP — and lazily, so four times the art costs half the load (2026-09-03)](#32-facings-for-a-voxel-8-for-an-shp-and-lazily-so-four-times-the-art-costs-half-the-load-2026-09-03)
- [The navy needed a second passability class, not a second pathfinder (2026-09-03)](#the-navy-needed-a-second-passability-class-not-a-second-pathfinder-2026-09-03)
- [`Underwater=yes` is a targeting class, not cover (2026-09-03)](#underwateryes-is-a-targeting-class-not-cover-2026-09-03)
- [The Directorate's "mid-game gap" was a seat, not a faction (2026-09-03)](#the-directorates-mid-game-gap-was-a-seat-not-a-faction-2026-09-03)
- [Separation only ran while a unit was walking (2026-09-03)](#separation-only-ran-while-a-unit-was-walking-2026-09-03)
- [A stuck detector that started its clock before the order arrived (2026-09-03)](#a-stuck-detector-that-started-its-clock-before-the-order-arrived-2026-09-03)
- [A ship needs a give-up that a tank does not (2026-09-03)](#a-ship-needs-a-give-up-that-a-tank-does-not-2026-09-03)
- [Lockstep multiplayer: one mutator, a two-tick delay, and a hash that argues (2026-09-03)](#lockstep-multiplayer-one-mutator-a-two-tick-delay-and-a-hash-that-argues-2026-09-03)
- [The AI's anti-air was a reaction, and a reaction is one sortie late (2026-09-03)](#the-ais-anti-air-was-a-reaction-and-a-reaction-is-one-sortie-late-2026-09-03)
- [Superweapon and paradrop clocks live over the battlefield, not on the sidebar](#superweapon-and-paradrop-clocks-live-over-the-battlefield-not-on-the-sidebar)
- [Right-drag always navigates; the right-click order is decided on release](#right-drag-always-navigates-the-right-click-order-is-decided-on-release)
- [Three player-reported fixes: paradrop icon, wall linking, debug combat](#three-player-reported-fixes-paradrop-icon-wall-linking-debug-combat)
- [Water has BODIES, and one house's cruiser needed the other house's radar (2026-09-03)](#water-has-bodies-and-one-houses-cruiser-needed-the-other-houses-radar-2026-09-03)
- [A ship was drawn at 8 bearings after the fleet had moved to 32 (2026-09-03)](#a-ship-was-drawn-at-8-bearings-after-the-fleet-had-moved-to-32-2026-09-03)
- [The top bar's icons are drawn, not typed (2026-09-03)](#the-top-bars-icons-are-drawn-not-typed-2026-09-03)
- [Three art gates that all failed by being SILENT (2026-09-03)](#three-art-gates-that-all-failed-by-being-silent-2026-09-03)
- [The walls were the same building twice, and one reference was Yuri's (2026-09-03)](#the-walls-were-the-same-building-twice-and-one-reference-was-yuris-2026-09-03)
- [A deployed GI was standing behind a crescent, not sitting in a pit (2026-09-03)](#a-deployed-gi-was-standing-behind-a-crescent-not-sitting-in-a-pit-2026-09-03)
- [`P_HUMAN` was answering three different questions (2026-09-03)](#phuman-was-answering-three-different-questions-2026-09-03)
- [The transport between two tabs is a BroadcastChannel, not a manager route (2026-09-03)](#the-transport-between-two-tabs-is-a-broadcastchannel-not-a-manager-route-2026-09-03)
