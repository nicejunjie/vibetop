...
**The quick row serves BOTH fields, and says which.** `Now · +/− · 1m/5m/30m/1h/5h` moves the row whose label is **lit**; `until` is lit by default, and focusing the `every` picker (or clicking its label — they are real `<label for>`, so the click, the keyboard association and the arming all come from one attribute) moves the aim to the cadence. `Now` **disables** rather than hides while the cadence is armed — an interval has no "now", and hiding it would reflow the row you are aiming at. Nudging the cadence inserts an ad-hoc option for the exact value (`5h 30m`) and prunes the previous one, so walking it with the buttons cannot grow a junk entry per tap.
