...
**The quick row keeps ONE meaning in both modes: it moves the time box.** It first shipped re-pointed at the interval — reusing it as the interval dialer it already was — and that was the wrong trade: the buttons stopped being predictable, because you could not tell what a tap would move without first checking the mode. So `Now · +/− · 1m/5m/30m/1h/5h` always drives the date field, and "Now, +5h" reads as *until 5h from now* exactly as it reads as *at 5h from now*. The interval owning a control of its own also reaches cadences the additive row never did conveniently (2h, 12h, a day). Editing an entry whose cadence predates the picker inserts an option for that exact value rather than snapping a running loop to the nearest listed one.
