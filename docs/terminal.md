...
**Repeat makes it a loop.** One toggle in the panel's action row, beside Schedule (a stroked SVG on `currentColor`, not an emoji — see `docs/design-decisions.md`). It reveals two rows, so a loop is the three values it actually has: **start · every · until**. The date field never changes meaning — it is when the message fires, called `at` for a one-shot and `start` for a loop's first run — which is why toggling Repeat stashes nothing (it used to swap that field between `at` and `until` with two parked values; the start row retired all of it). `every` is a `<select>` (1 min … 1 day, default 5h — the token-limit case) and `until` its own `datetime-local`. Turning Repeat on seeds `until` to +24h and leaves `start` alone — i.e. at **now**, which is what the panel seeds on open, so the first run goes out on the next sweeper tick. Repeat briefly pushed `start` one interval out instead (a loop is usually turned on while rate-limited, so firing at once burns a run); that was defensible while the start was *invisible*, but with the row on screen it only meant the field showed a time nobody chose. "Start later" is one tap on that row.
