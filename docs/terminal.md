...
**🔁 makes it a loop.** One toggle in the panel's action row, beside Schedule — and **no new input**, because the `+/− · 1m/5m/30m/1h/5h` quick row was *already* an interval dialer, it just folded the offset into a date. With 🔁 on it stops folding: the offset stays the interval (a read-only `every 5h` chip), which frees the one `datetime-local` to mean **until**, relabelled `at`→`until` so the shared field is never ambiguous. `Now` hides (an interval has no "now"); the `+/−` toggle keeps working, on the interval. The two meanings are parked in separate JS vars, so flipping 🔁 back and forth loses nothing. The client posts `{at: now + every, every, until}` — **the first run is one interval out**, not immediate: you turn a loop on because you are rate-limited *now*, so firing at once would burn a run. Interval carry-over is measured from the **current minute**, not the current instant — the field only resolves to the minute, so a live-clock baseline turns "+5h" into `every 4h 59m` and drifts the loop a minute a run.
