...
- `vibetop-manager.service` — threaded Python HTTP server on `127.0.0.1:7680` (runs as root; `ThreadingHTTPServer` so a slow request — a multi-GB upload, health probes — can't block the status polls). Manages terminal lifecycle and provides system status. Endpoints:
  ...
  - Opening an office/video file in Files (`filesx.html`/`filesx-core.js`): a single click/**tap** only **selects**, a **double-click / double-tap** opens (detected on the click events). Touch uses the same double-tap path as mouse (a single tap must never play a video). A video opens in the **Video player** — `APPS.video` (`/video.html`), a `hidden:true` app that never appears in the Start menu and is registered only so the taskbar/title bar can render it; Files opens it by pushing `'video'` into `openApps` directly. (Why a custom player rather than an inline preview, and why it must reach files **outside** `~`: `docs/design-decisions.md`.)
...
The client posts `{at, every, until}` straight from the three rows. The server always took these as three independent values (validating `until >= at` and capping total runs), so exposing the start was a front-end change only.
...
- The proxy sends the URL to `POST /api/browser/open`, which starts the xpra display if needed, waits for its main Chromium window, then runs `chromium <url>` on that display. The wait keeps a cold-start URL from racing Chromium's profile lock.
