# Iron Frontier (`landing/rts.html`) — art & presentation gap audit vs Red Alert 2

Audited 2026-09-02 against `landing/rts.html` @ 17267 lines (HEAD `0dfab90`), served
...
| **No recoil** on any gun | RA2 tanks visibly rock back on firing (`Recoilless=yes` is called out as the *exception* on `[GAPILL]`, art.ini:2113) | `grep -c recoil landing/rts.html` = 0 | minor | M |
...
| **`fsheet.js` clips the entire Collective row** — the 2600 px canvas only fits the Directorate set plus four Collective items, so the harness silently under-reports | `tools/rts-art/fsheet.js` (fixed `height: 2600`) | `fsheet.png` bottom, `fsheet_col.png` |
