...
- **SOURCE:** `allied-ore-refinery-idle.png` (in-game gif), `allied-ore-refinery-anim*.png` (clean SHP rip, remap red). Aspect 1.38.
- **READ (mirrored, dock on +gy = down-left):** a fat horizontal cylinder whose OPEN MOUTH faces the viewer-left: nested C-shaped hoops (silver rim, ivory, lavender, ivory, house hoop) wrap its right side and top and open left into a navy cavity with a grey slatted floor, a tan hoist and the amber ore slot. The rails leave the cavity down-left to the dock tile. The tall straight silver stack (cap with a dark bore, groove, neck, house collar, ribbed bulge) stands over the hoops' top; the short one on a ribbed silver bulge over an olive ribbed skirt at the right corner with a pipe loop. House-coloured ported spine (three ports) from the tall collar down-left to a jamb block at the mouth's left. Charcoal deck, white rails, chevron patch at each outer corner, a house tank with a white band on either side, three stepped grey blocks under the mouth. No steam.
- **MOST COMMON MISTAKE:** reading the hoops as a closed face (a wheel, a half-disc): they are rings round an open cylinder and the harvester drives INTO it. See `docs/design-decisions.md`.

### refinery — Soviet Ore Refinery (Collective)
- **SOURCE:** `soviet-ore-refinery-anim-last.png` (clean SHP rip; the magenta blob is the shadow index), `soviet-ore-refinery-scene.png`. Aspect 1.10.
- **READ (mirrored, pit on +gy = down-left):** near furnace down-right: a bell-profile iron cone (lit olive-brown left third, black right) with six pale organ pipes (elbow into the iron, foot on the pad) into a black chimney with a pale collar at its foot and a pale ring under a flared black lip; back furnace up-left: taller, two-stage (lower cone, pale collar, upper cone, collar, chimney). Between them the house-coloured hopper slab with the inclined slatted conveyor bridge rising toward the near chimney and two fanned slatted grilles. Pale concrete octagon under the near cone, big dished pit (pale rim, dark groove, khaki dish) with a dark ladder from the block's foot, pale ramp slab off the pit to the dock tile with a house lamp on each edge, small tank behind the back cone. No leg struts.
- **MOST COMMON MISTAKE:** reusing the Allied massing in red, or standing the cones on struts — they sit on the pad ringed by pipes.
