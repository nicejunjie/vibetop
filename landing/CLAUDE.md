...
- `coach.js` — shared coach-tip banner (`window.vibeCoach(tips, opts)`): show-every-open-until-×, max-10 cap, versioned keys, one-at-a-time, rotation. Loaded on the desktop shell + X11 Launcher and injected into `/tN/` pages before `terminal-kbd.js`. Tips live on 4 surfaces (terminal, Files, Browser, cross-device ⏻) — see the root `CLAUDE.md`
