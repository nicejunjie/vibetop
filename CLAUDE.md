...
**Re-claim the terminal shape (`terminal-kbd.js`)** — terminal N is **one shared `claude-session` PTY**, so its `rows×cols` belong to whichever device resized last; after you switch active device the TUI inside renders at the *other* device's shape (phone-shaped text on the desktop, desktop-shaped on the phone) — and the desktop only auto-re-fits on app *activation*, which misses the case where the phone steals the size while the desktop terminal stays active. A gesture re-sends **this** device's terminal size to the PTY: **double-click** (desktop) / **double-tap** (touch). The fix is `claimSize()` — it **nudges xterm's size by one row and restores it** (`term.resize(c,r-1); term.resize(c,r)`) because ttyd only emits a resize when its computed dims actually *change*; the restore re-sends this browser's real dims, the daemon `TIOCSWINSZ`es the shared PTY, and the TUI redraws at this device's shape (the other device then sees mis-shaped output until *it* re-claims — symmetric, since the PTY can't be two shapes at once). The desktop handler lives **before** `terminal-kbd.js`'s non-touch early-return (the rest of the file is touch-only). On touch the double-tap is detected by touch **duration** (`<250ms`), **not** `moved` — the first tap raises the keyboard and the layout shift while it animates in drifts the finger, which a movement-based check would misread as a scroll and drop the second tap; a brief **"↔ resized"** toast confirms it fired, and it blurs the keyboard so reshaping doesn't leave it up.
