...
| **RTS game (`apps/games/rts/rts.html`): the standing "true RA2 experience" requirement + roadmap** | `apps/games/rts/docs/roadmap.md` (then `docs/ra2-art-plan.md`); unit art lives in `apps/games/rts/art/units/<class>/<kind>.js` and `rts.html` is BUILT from `rts.src.html` + those files (`tools/rts-build.py`) |
...
|---|---|---|
| `server/` | `/api/` | The manager — the backend for EVERY app, plus the generated nginx site config |
| `apps/everyday/terminal/` | `/t1/`..`/t50/`, `/terminals/` | Dynamic persistent bash terminals (ttyd + vibetop-session) |
...
| `apps/everyday/files/` | `/files.html` | The Files app — vibetop's own file manager, browsing from `/` as the authenticated user through a per-user file agent |
...
- **One backend for every app.** `server/terminal-manager.py` (~7k lines) is the
  whole server side — not just terminals. It sat in `terminal/` until 2026-09-03,
  which is why the file is still named `terminal-manager.py`; the DIRECTORY now
  says what it is. A single stdlib `http.server` running as
...
- **Root proxies, per-user daemons act.** The manager never touches a user's files
  ...
  ttyd (terminals), xpra. Unix permissions are the entire authorization
...
- **nginx is the front door and the only gate.** Every protected location carries
  ...
  headers nginx needs (`X-Term-Port`, `X-App-Port`) so `/tN/`, `/browser/` and
  `/x11-display/` route *by identity*. The main site config is **generated by a heredoc in
  `server/install.sh`** (not a checked-in file); sub-projects ship their own
  fragments in `*/nginx/*.conf`.
- **No build step** — with one exception. There is no bundler, no root
  `package.json`, no transpile; installers `cp` HTML/JS verbatim. The exception
  is the RTS page: `apps/games/rts/rts.html` is **generated** (gitignored,
  read-only) from `rts.src.html` + one file per unit under `art/units/**` by
  `apps/games/rts/tools/rts-build.py`, which `run-tests.sh` and
  `shell/install.sh` run first. Edit the sources, never the output. Each frontend is one self-contained file
  ...
  those are content-hash cache-busted, not versioned by hand.
