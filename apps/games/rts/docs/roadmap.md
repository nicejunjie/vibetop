...
5. **A unit's art is its own file, and nothing is built.** Edit
   `apps/games/rts/rts/units/<class>/<kind>.js` — a plain script declaring one
   `function drawX(C)` over a single context object, listed as a
   `<script src>` in `rts.html` — or the subsystem file that owns whatever else
   you are changing (`rts/*.js`, `rts/bake/*.js`, `rts/ui/*.js`). There is no
   generator and no output file, so a saved file is live on the next reload.
   Play it straight from the repo by **double-clicking
   `apps/games/rts/rts.html`**, or serve the directory with
...
- ☑ Edge scrolling from a black navigation GUTTER round the whole frame —
  outside the build panel and the command bar; neither the map's border nor
  the panels scroll — and the camera clamp follows the map diamond a few
  cells out so any edge can be centred (2026-09-11).
