...
All eight were recorded here as CEILINGS, reducing to **four** underlying facts:
elongation, the un-normalised IoU, the dog, and the cross-group bake scale.

> **One of the four was not a fact about the art (2026-09-06).** "Elongation"
> was the correlate of an artefact in the `peerVsSelf` estimator itself, proven
> on plain filled rectangles and repaired. Three rows moved — `peerVsSelf.total`
> 6 → 2, `.vehicle` 1 → 0, `.naval` 5 → 2 — and **nothing else in the gate did**
> (3 of 57 metrics). The two that remain are one real pair and are marked REAL
> below, not CEILING: they want art. The other three ceilings stand.

| metric | value | target | verdict | evidence |
|---|---|---|---|---|
| `peerVsSelf.total` | ~~6~~ **2** | `<= 0` | **REAL — one pair** *(was CEILING)* | **This row was 6 and the verdict was CEILING; both were the metric's fault and both are corrected (2026-09-06).** The old estimator, `cross(k,p) - self(k) > 0`, charged a unit for its own silhouette's spread across bearings and gave it no credit for the peer's, so a compact peer near the cloud's centre always won. `tools/peer-vs-self-control.js` proved it on plain filled RECTANGLES — `-0.0786` against the V3's measured `-0.0787`. The row is now the unbiased two-sample statistic `self(k) + self(p) - 2*cross(k,p) < 0`; corr with aspect swing over the 13 vehicles fell from **+0.545 to -0.049**, and `dread, squid, sub, v3` — four of the six — cleared with **no art change at all**. Only `destroyer` and `aegis` survive, and they are one pair counted once from each side. `docs/design-decisions.md`, "the missing term was the PEER's own spread". |
| `peerVsSelf.naval` | ~~5~~ **2** | `<= 0` | **REAL — art work needed** *(was CEILING)* | **The "it measures ELONGATION" verdict was right about the symptom and wrong about the remedy: the metric was repaired, not the fleet.** Three of the five were the artefact and cleared untouched. What is left is `destroyer | aegis` at mmd2 **-0.0337** — the only negative on the board, against **+0.0281** for the next-closest pair — and it is a REAL finding that no other metric in the gate catches: their same-bearing IoU is **0.724**, sneaking under `iou.sameFactionOver75`'s 0.75 ceiling, while `legibility.js` independently ranks `destroyer | aegis` its **worst pair in all three zoom windows** (48.5 at ZMIN against a 40.1 threshold). Their bboxes track each other at every octant (70x51/67x43, 89x42/82x37, 46x57/48x49). **The fix is SUPERSTRUCTURE, not proportion** — both hulls are inside RA2's aspect band and must stay there; shortening or rounding either is the tugboat error the aspect gate exists to catch. |
| `peerVsSelf.vehicle` | ~~1~~ **0** | `<= 0` | **MET** | **The caveat this ledger recorded rather than hid was correct, and it was the row that bit.** It read: "the argument rests on the six being the six longest on the board, mean aspect 2.68, and the V3's broadside aspect is 1.455 — well below that mean and below three *clean* naval hulls... if anyone ever brings new evidence to a `peerVsSelf` row, this is the one row where it could bite." Elongation was the correlate; **swing** was the cause (`corr(aspectSwing, selfIoUCross) = -0.823` over the 13 vehicles) and the V3 swings 2.61. An entire measured pass could not close this row — the missile **deleted entirely** still left it flagged — and the repaired estimator closed it for free. Its nearest peer is now the War Miner at mmd2 **+0.0281**. |
