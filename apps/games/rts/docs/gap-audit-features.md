...
| 19 (rest) | **`ElitePrimary` DONE.** 34 anchored `ElitePrimary=` lines, **26** of them on units we field, all 26 now transcribed as a sparse `elite: {}` per weapon, baked into a complete weapon at load and swapped in by `eliteOf()` at whichever weapon `weaponFor` resolved (so the Giant Squid's `SquidGrabE`, which sits on our SECONDARY, is not missed). Four warheads added with their real `Verses=` rows — `RHINAPE`, `GRIZAPE`, `ApocAPE`, `HowitzerWH`; `KTSTLEXP` recorded as a **substitution** (its row is byte-identical to `[BlimpHE]`, so an elite Kirov's bomb differs only in explosion art) and `SSA`/`ARTYHE` were already present. Three prerequisites had to land first: **(a)** `Burst` was spelled three ways — a field on the Dreadnought, pre-multiplied into `dmg` on the Apocalypse (200 = 100x2), its Mammoth Tusks and the IFV, and folded into `ammo` on the Harrier — and `[120mmE]`/`[120mmxE]`/`[HoverMissileE]` all CHANGE Burst, so the folds were unfolded (`fireGround` and garrison fire were also silently dropping Burst); **(b)** the stacking rule was settled from the ini — the `[General]` scalars apply **ONCE**, not per rank, and the elite weapon **stacks** on them, so the row's own "double-buffed" blocker was never real (`docs/design-decisions.md`); **(c)** `ROF` varies per unit, so units carry `rofAt` — without it `[HARV]`'s elite gun is a net 0.67x and an elite War Miner is weaker than a veteran one. Guarded by a test that walks every mapped weapon and asserts elite DPS >= veteran DPS, proved to bite by deleting `rofAt` | this commit |

| 33 | **Split: half the row is unsupported, half is real — and the real half has since SHIPPED.** `MultiplayerAICM=400,0,0` is described in rules.ini only as "Multiplayer AI Coefficient of Money (Genius, Smart, Easy)" — a per-difficulty lump, which is exactly what the code pays once at match start on Hard. The row calls that a defect without saying what the correct behaviour would be, so **no change made**. The trigger track record is **done** (`cf16d7f`): `AI_TRIG_WIN = 20` / `AI_TRIG_LOSS = -50` / `AI_TRIG_COEF = 1` match rules.ini's `AITriggerSuccessWeightDelta=20`, `AITriggerFailureWeightDelta=-50` and `AITriggerTrackRecordCoefficient=1` exactly; `aiRec()` records a win at the team's objective and a loss on both the disband and the wipe path, and `aiTeamPass` folds the record into each trigger's weight with a floor of 1 | `cf16d7f` |

| 26 (rest) | **Cliff seam-matching — CANDIDATE TESTED AND REJECTED (2026-09-04).** The proposal was to taper `crest`/`crest1` to zero at the cell's end vertices, exactly as `pt()` already tapers the jut, so the two sides of a shared corner would agree instead of one skyline ending 10 px up while its neighbour began at 0. It was written, and this time it was **rendered and measured** rather than reverted unseen. Harness: Chokepoint Pass with `localStorage['vibetop:rts:map']='choke'` set BEFORE Start (`__rtsTest.startWith` sets `headless = true`, which is why three earlier attempts came back as shroud), `g.seen.fill(1)`, the camera placed on the densest cliff neighbourhood found by scoring every cliff cell's 9x9 window — the wall is DIAGONAL (`setTM(g, x, MAP-1-x+k, ...)`), so a horizontal run finds 3 cells at the map edge — and the seed pinned by overriding `Date.now`/`performance.now` across the synchronous Start click, without which a before/after is two different cliffs. **Result: 1.34% of pixels change, all inside the crest band (bbox rows 313-368), and the skyline discontinuity does not move — topmost-rock-pixel per screen column gives mean step 0.91 px before and after, steps >= 4 px 30 before and 29 after, max 10 both.** The taper only reaches columns whose `t0`/`t1` sit inside the outer 20% of a cell, so it dips those columns without matching the neighbour, and the panel reading survives. Reverted. **The remaining defect is real and needs seam-matched CONSTRUCTION between cells — adjacent cells agreeing on a shared crest height before either bakes — not a per-cell taper.** | — |

| 27 (rest) | **Kirov gondola — CLOSED, and the row's premise was wrong (2026-09-04).** The row recorded "measured 0 px of daylight in every bearing" and left §2.4's "gondola separated below the envelope by >= 4 px" open, calling it a trade against the size target at ~6% of broadside aspect per 4 px. **Re-measured off the live bake: the broadside is 147x66, aspect 2.227 against RA2's [ZEP] 139x62 = 2.24, with 7 px of daylight — and all 32 frames carry a column of >= 4 px.** The 0 px reading predates `KGAP`, which landed with a comment citing this very section. **The trade is also imaginary in the direction that matters:** at `KGAP` 5.8 the daylight profile is IDENTICAL (6 columns, max 7 px, median 1) while the sprite grows to 147x70 and the aspect falls to 2.100 — the gap is bounded by the pod/envelope geometry, not by the drop, so paying height buys nothing. `KGAP` stays 3.4. Both §2.4 numbers now have a regression guard in `tests/e2e/tests/rts.spec.js` ("the Kirov keeps RA2 proportion AND hangs its gondola clear"), which the art gate cannot provide — it reads alpha silhouettes at eight bearings and says nothing about internal daylight. Proved by raising `KGAP` to 5.8 and watching it fail on aspect | this commit |

| R1 / §2 | **ZMIN raised 0.55 -> 0.75 — the floor every other art number rested on (2026-09-04).** `unit-identity-reference.md` R1 says RA2 authored its identity spikes at a 2 px floor for a renderer that NEVER scales, so below ~0.75 an RA2-faithful feature is under 2 device px and "no other item on this list is verifiable in play". `unit-redesign-plan.md` §2 offered three routes and chose (b), authoring every spike past the 3.64 px floor. That work is done — and it was **not enough**: measured with `tools/legibility.js`, four infantry pairs still sat under the friend-vs-foe threshold at 0.55. Swept to find the crossover rather than guessing: **0.55 -> 4 pairs under (ivan\|spy 17.0 worst), 0.65 -> 2, 0.75 -> 0 in EVERY group (vehicle, infantry, air, naval), 0.85 -> 0.** 0.75 is the measured minimum; §2's option 3 guessed 0.85 and costs more view than it needs to. The cost is bounded — the player still sees ~1.8x RA2's field of view at full zoom-out against 3.3x before, and the minimap carries the strategic overview exactly as it does in RA2. Verified by rendering a crowd of eight infantry at the clamp with no page errors. Guarded in `rts.test.js` ("ZMIN stays at or above the measured legibility floor"), proved by setting it back to 0.55 and watching it fail. **Remaining, and now the only pair anywhere under a threshold: `aegis \| squid` 27.6 at zoom 1 against 32** | this commit |

| new | **`aegis \| squid` — CLOSED, and this row's own conclusion was the reason it stayed open (2026-09-05).** The pair is gone: 27.6 -> the naval minimum in that window is now 43.5 against a 35.0 floor, and the game has no confusable pair left anywhere. **The row said "neither is the size" and compared the wrong numbers.** It checked the PLAN geometry — `G.aegis` `L41/W17` = 2.41 against `[AEGIS]` 91x35 = 2.60 — but a plan is not a sprite: everything a ship carries above the deck lands in the rendered height and in no plan number at all. Measured on the rendered broadside frame, where RA2's 91x35 is measured, the Aegis was **78x58, aspect 1.34 against 2.60** — a 20-unit deckhouse slab on a 5.6-unit freeboard, a cruiser drawn as a tugboat. Five of the seven hulls with an RA2 sprite were outside a +-20% band (the Typhoon at 0.40, the Dreadnought 0.50, the Aegis 0.52); the Squid's mantle and the Dolphin's body were `g.ellipse` calls at fixed SCREEN radii, identical at all 32 bearings, so neither animal ever foreshortened. All seven are now inside the band, worst 0.194, and `aspect.navalOutsideRA2Band` in `tools/art-metrics.js` holds it — the first art metric with an EXTERNAL reference, which is what no ensemble metric could ever have. Traded `peerVsSelf.naval` 1 -> 9, deliberately and re-recorded: `selfIoU` is `1/(2a-1)` for aspect *a*, so it rewards round ships. Full reasoning in `docs/design-decisions.md`. | closed |
