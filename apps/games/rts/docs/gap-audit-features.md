...
| new | **`aegis \| squid` 27.6 against a 32 threshold at zoom 1 — the only pair anywhere still under a floor, and it is a REAL reading.** §2.4 calls the Giant Squid "the only unit whose outline is not a machine", so reading like a cruiser is a defect by its own spec. The obvious explanation was measurement: `tools/legibility.js` composes EVERY unit in owner 0's paint, while `aegis` is dir-only and `squid` col-only — a player takes one side and the AI the other, so the two are always on opposing sides and can never share a colour, and comparing the cross-colour pictures that DO occur should have cleared it. **Tried, and disproved.** The pair stayed at 27.6 (`near()` is luminance-weighted, so an owner-colour swap barely moves it) and taking the harder of the two cross-colour arrangements pushed `conscript \| tanya` from 32.9 to 23.8, opening a new ZMIN confusable that the shape work had just closed. Reverted. **The paint is not the problem and neither is the size:** the bake dims are already RA2-proportioned — squid `L62/W17` = 3.65 against `[SQD]` 117x30 = 3.90, aegis `L41/W17` = 2.41 against `[AEGIS]` 91x35 = 2.60 — and the squid is correctly the longer of the two, as in RA2. What is left is silhouette work on §2.4's own terms: "zero straight edges; >= 4 tentacles resolvable at 3 px each" against the Aegis's "radar panel >= 8x8 px, vertical; explicitly no barrel" | — |
