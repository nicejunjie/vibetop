...
ring(-.31,2.3,'#4d5050');ring(.11,2.5,'#55595a');ring(.52,2.2,'#666968');ring(-.3,.75,'#b1ae89');ring(.12,.75,'#aaa886');ring(.53,.7,'#bbb998');
var rail=[];for(i=0;i<2;i++){g.beginPath();for(j=0;j<=16;j++){t=-.42+j*1.05/16;p=P(t,.87-i*.13);j?g.lineTo(p[0],p[1]):g.moveTo(p[0],p[1])}g.strokeStyle=i?'#55574f':'#9b9a83';g.lineWidth=.8;g.stroke()}for(t=-.35;t<.63;t+=.18){var u=P(t,.86),v=P(t,.73);g.strokeStyle='#4b4d49';g.beginPath();g.moveTo(u[0],u[1]);g.lineTo(v[0],v[1]);g.stroke()}
