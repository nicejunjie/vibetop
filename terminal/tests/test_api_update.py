...
def test_update_already_up_to_date(client, mgr, stubs, monkeypatch):
    g = GitFake()
...
def test_update_clean_fast_forward(client, mgr, stubs, monkeypatch):
    g = GitFake()
...
def test_update_restart_when_manager_module_changes(client, mgr, stubs, monkeypatch):
    g = GitFake()
...
def test_update_rsync_redundant_tree_is_reset(client, mgr, stubs, monkeypatch):
    g = GitFake()
...
def test_update_genuine_dirty_is_blocked(client, mgr, stubs, monkeypatch):
    g = GitFake()
    g.dirty = " M terminal/terminal-manager.py"
    g.matches_upstream = False                   # real local edits, not upstream
    monkeypatch.setattr(mgr, "_git", g)
    status, body = client.post("/api/update", {})   # force omitted
    assert status == 200 and body["ok"] is False
    assert body["blocked"] == "dirty"
    assert "terminal-manager.py" in body["dirty"]


def test_update_force_stashes_then_updates(client, mgr, stubs, monkeypatch):
    g = GitFake()
    g.dirty = " M terminal/terminal-manager.py"
    g.matches_upstream = False
    g.changed = ["landing/desktop.html"]
    monkeypatch.setattr(mgr, "_git", g)
    status, body = client.post("/api/update", {"force": True})
    assert status == 200 and body["ok"] is True
    assert _step(body["log"], "stash local changes") is not None
    assert any(c[0] == "stash" for c in g.calls)


def test_update_fetch_failure_reports_cleanly(client, mgr, stubs, monkeypatch):
    g = GitFake()
    g.fetch_ok = False
    monkeypatch.setattr(mgr, "_git", g)
    status, body = client.post("/api/update", {})
...
def test_update_check_reports_behind(client, mgr, monkeypatch):
    g = GitFake()                                # head != remote, log -> 1 commit
    monkeypatch.setattr(mgr, "_git", g)
    status, body = client.post("/api/update/check", {})
    assert status == 200 and body["ok"] is True
    assert body["behind"] == 1
    assert body["commits"][0]["subject"] == "Some commit"


def test_history_clear(client, mgr, monkeypatch):
    monkeypatch.setattr(mgr, "_git", GitFake())
    # Seed a history entry, then clear it.
    mgr._append_update_history({"time": 1, "event": "updated"})
    status, body = client.post("/api/update/history/clear", {})
    assert status == 200 and body["ok"] is True
